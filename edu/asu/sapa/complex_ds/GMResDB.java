...
/*		for(int i = 0; i < num; i++)
    ...
    //changed by bharat -applies only instntaneous effects of functions
    ...
    public void update(ArrayList aList) {
		...
		for( i = 0; i < aList.size(); i++ ) {
                    if(!(aList.get(i)  instanceof FunctionEvent))continue;
		    e = (FunctionEvent) aList.get(i);
                    dur  = e.getTime();
                    ms = e.getMySet();
		    fID = ms.getLeftSide();
		    value = getMFValue(ms.getRightSide(), dur);
