...
public class GState {
    ...
    public GState(GState s) {
     ...
     public boolean isTrue(int dpIndex){
        GroundDerivedPredicate gdpred = (GroundDerivedPredicate)dPreds.get(dpIndex);
        return gdpred.isTrue(this);
    }
    public void figureDPreds(){
        for(int i=0; i<dPreds.size(); i++){
            if(isTrue(i)){
                    truefalse[i]=true;
