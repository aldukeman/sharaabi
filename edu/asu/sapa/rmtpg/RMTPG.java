...
/*		for (int l=0;l<actPlusOne;l++) {
		...
		for(i = 0; i < numAct; i++) {
		    ...
		    if (action.getDType()) {
			    ...
			    actLevel[i] = new ActLink(i,duration,action.numInsPrecond(),action.numDer(),
						      action.numAdd(), cost);
		    } else {
		    	// duration on action is dynamic, so put GMathForm into ActLink.
			    actLevel[i] = new ActLink(i,action.getDDynamic(),action.numInsPrecond(),action.numDer(),
					      action.numAdd(),cost);
		    }
...
/*		    if (numPrecond != action.numInsPrecond()) {
		...
		for(i = 0; i < numAction; i++) {
		    initialNumPredNotSat[i] = actLevel[i].numPrecond();
                    initialNumPredNotSat[i] -=actLevel[i].selfSupport();
                    initialNumPredNotSat[i] -= actLevel[i].numDer();
		}
    }
...
//		    ce = (CostEvent) eventQueue.get(i);
     ...
     */
    public GAction[] applicableActions(StateInfo state, ArrayList gActions,Grounding g) {
    		...
    		for (int j=0;j<numSupportedActions;j++) {
        		...
        		if (--numPredNotSat[action.actID] == 0) {
        			GAction possibleAction = (GAction)gActions.get(action.actID);
                                //System.out.println("Considering"+possibleAction);
        			if (state.applicable(possibleAction,g)) {
						...
						actionList.add(possibleAction);
                                                //System.out.println("Applicable action"+possibleAction.getID());                                                
        			}
        		}
    		}
    	}
