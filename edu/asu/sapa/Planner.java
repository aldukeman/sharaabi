...
//	    oppOCPlan = optimalPostProObj.getOCPlan();
    ...
    private int solutionSearching() {
        ...
        while (true) {
            ...
            if (sp != null) {
                //System.out.println("State is" + sp.getCurrentPredDB().getAllPred());
                //System.out.println("State Actions applied" + sp.getActions());
                //System.out.println("At times"+sp.atime);
            }
            ...
            GAction[] applicableActions = rmtpg.applicableActions(sp, gActions,groundInstance);
            //System.out.println("The current time is kgh" + sp.getCSTime());
            //System.out.println("The applicable actions are...");
           /* for (int ki = 0; ki < applicableActions.length; ki++) {
                System.out.print(applicableActions[ki]);
            }*/
            ...
            for (i = 0; i < applicableActionsLength; i++) {
                ...
                if(act.getID()==sp.getLastAction())continue;
			...
			/*if( haFlag ) {
                ...
                tempSP.update(act);
                //System.out.println("State updated with" + act.getID());
               if (tempSP.violatesValidConditions(groundInstance)) {
                    //System.out.println("Eliminating state with action "+act.getID());
                    continue;
                }
                ...
                if (propResult > 0) { // Propagation successed, now get the real value
                    ...
                    if (heuristicValue == 0) { // Solution found
                        float uptoTime = tempSP.getLastVerificationTime();                        
                        if(tempSP.getLastEventTime()>uptoTime){
                        aDate = new Date();
                        time2 = aDate.getTime();
                    ...
                    if(heuristicValue!=0 || tempSP.getCSTime()==0){
...
//    	System.out.println("finished: current state: " + numNewStates + " average difference: " + (double)totalNewDifferences / (double)numNewStates);
            ...
            if (sp.canMoveForward()) {
                ...
                if ((!deadlineFlag)
                    ...
                    if (!tempSP.violatesValidConditions(groundInstance)) {
                        ...
                        tempSP = new StateInfo(sp);
                        if (tempSP.setTimeToEarliestEvent()) {
                            if (!tempSP.violatesValidConditions(groundInstance)) {
                                stateMan.addStateInfo(tempSP);
                                generatedState++;
                            }
    ...
    private class TilP{
     int predId; //the timed initial literal
     float trueTime;
     float falseTime;
     boolean populateFalseTime(int p,float t){
         if(predId==p){
             if(falseTime>0)return false;
             else{
                 falseTime = t;
                 return true;
             }
    ...
    public int solve(Domain domain, Problem prob) {
        ...
        if (debug_mode == false) {
           ...
           /* System.out.println("\n<<< Ground Actions >>>");
            ...
            for(i=0;i<groundInstance.numPred();i++){
                System.out.println(i+"The predicate"+groundInstance.getPredSig(i));
            }*/
        }
        ...
        /*TilP tilps[] = new TilP[numEvents/2];
        int tilcounter = 0;*/
        for (i = 0; i < numEvents; i++) {
            e = (Event) tempA.get(i);
           /* if(e.getNeg()==false){
                tilps[tilcounter] = new TilP(e.getPred(),e.getTime());
            }else{
                for(int it=0; it<tilcounter; it++){
                    if(tilps[it].populateFalseTime(e.getPred(), e.getTime()))break;
                }
            }*/
        ...
        /*for(i=0; i<tilcounter; i++){
            //code to implement tilps
        }*/
        // End Testing
