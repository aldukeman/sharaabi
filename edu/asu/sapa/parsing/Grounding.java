...
public class Grounding {
    ...
    ArrayList derSigList = new ArrayList(); //derived predicate signatures
    ArrayList gactions;
    public ArrayList gdpreds = new ArrayList();
...
//    GAction instance;
    ...
    public ArrayList getGDPreds(){
        return gdpreds;
    }
    public int numGDPreds(){
        return gdpreds.size();
    }
    ...
    /*Bharat*/
    public int getPredIndex(String s){
        for(int i=0; i<predSigList.size(); i++){
            String psig = (String)predSigList.get(i);
            if(psig.equals(s))return i;
        }
        return -1;
    }

    public int getDPredIndex(String s){
        for(int i=0; i<derSigList.size(); i++){
            String dpsig = (String)derSigList.get(i);
            if(dpsig.equals(s))return i;
        }
        return -1;
    }


    public void getAllGroundDerivedPredicate(DerivedPredicate template) {
                int size = template.predSize(), pointer, i, j;
		int[] index = new int[size], numObj = new int[size];
		ArrayList[] objSet = new ArrayList[size];
		String type;
		String derSig; // Signature of the pred = concatenation of all objects
		...
		pointer = size -1;
		while(true) {
		    derSig = new String(template.getName());
                    GroundDerivedPredicate gp = new GroundDerivedPredicate();
                    GroundPredicateFormula gf = new GroundPredicateFormula();
                    ArrayList gpparams = new ArrayList();
		    for(i = 0; i < size; i++) {
				derSig += "*" + (String) objSet[i].get(index[i]);
                                gpparams.add((String) objSet[i].get(index[i]));
		    }
                    gp.processParams(gpparams, template, gf,this);

		    derSigList.add(derSig);
                    gp.setID(derSigList.size()-1+numPred());
                    gdpreds.add(gp);
    ...
    private void getAllGroundPredicate(Predicate template) {
		...
		String predSig; // Signature of the pred = concatenation of all objects
                //System.out.println("Number of params"+size);
	
		if( size == 0 ) {
		    predSigList.add(new String(template.getName()));
                    //System.out.println("the predicate being added"+template.getName());
		    return;
		}
		...
		for(i = 0; i < size; i++) {
		    ...
		    type = template.getObj(i);
		    objSet[i] = (ArrayList) aProb.getObjectMap(type);
                    //System.out.println("Type"+type+"***Predicate"+template.getName());
		...
		while(true) {
		    ...
		    for(i = 0; i < size; i++) {
				//System.out.println("Object set index"+index[i]+"actual size   "+objSet[i].size());
    ...
    /***Predicate formula getting grounded***/
    ...
    public StateManager getStateManager(Domain aD, Problem aP) {
		...
		for(i = 0; i < aDomain.numPred(); i++) {
		    ...
		    getAllGroundPredicate(tempPred);
                ...
                for(i = 0; i < aDomain.numDPred(); i++) {
		    dpred = aDomain.getDPred(i); // get the template
		    getAllGroundDerivedPredicate(dpred);
		}
	...
	if(index < 0) {
            //check in derived predicates
            index = derSigList.indexOf(signature);
            //System.out.println("looking for index of"+signature+ "~~~in a list of size"+ derSigList.size());
            if (index < 0){
	...
	return new Integer(index);
     ...
     private Integer groundPredicateSpec(Hashtable objMap, Predicate template,nonw st) {
	Object obj, objTemp;
	String signature = new String(template.getName());
	int index;
    ...
    public void getObjectSet(ArrayList types, ArrayList ins,
			   int index) {
        if(types.get(index) instanceof String)System.out.println("Its a string");
        
        //System.out.println("The index"+index+"The type"+(String)types.get(index));
	String type = (String) types.get(index);
		    ...
		    for( ; j < aArrayList.size(); j++) {
				...
				for(k = 0; k < aTemplate.numPrecond(); k++) {
				    time = aTemplate.getPrecondTime(k);
                                    nonw st = new nonw();
                                    st.t = 0;
				    pred = groundPredicateSpec(objMap, aTemplate.getPrecond(k),st);
...
//			System.out.print(instance);
		...
		if( noDurativeAct ) {
		    for(i = 0; i < actSize; i++) {
                ...
                gactions = gActionList;
	
		return gActionList;
	}


public int numGActions(){
    return actSigList.size();
}
/* get all actions that have this predicate in
 * their add effects*/
public ArrayList getSupportingActions(int gp){
    ArrayList a = new ArrayList();
    for(int i=0; i<numGActions(); i++){
        GAction ga = (GAction)gactions.get(i);
        if(ga.indexAdd(i)>0)a.add(ga);
    }
    return a;
}

  public ArrayList getIncActions(int funcId){
        ArrayList a = new ArrayList();
        for(int i=0; i<numGActions(); i++){
        GAction ga = (GAction)gactions.get(i);
        int k = ga.indexSet(i);
        if(k>0){
            if(ga.isIncrement(k))a.add(ga);
        }
    }
    return a;
