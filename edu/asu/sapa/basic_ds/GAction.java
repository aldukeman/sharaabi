...
public class GAction {
    ...
    boolean[] derived; //indicates whether the precondition is a deriveed predicate
    ...
    // or "over all"). Thus, excluding "at end" condition.
     ...
     public boolean isIncrement(int i){
        GMySet g = setList[i];
        return g.isIncrement();
    }
    ...
    public boolean selfSupported(int i){
        if(i>this.numPrecond())System.exit(-1);
        int precond = preconds[i];
        for(int j=0; j< this.numAdd();j++){
            if(addTime[j]==0){
                if(precond==addList[j])return true;
            }
        }
        return false;
    }
    ...
    public GAction(int numPrecond, int numTest, int numDel, int numAdd, int numSet) {
		...
		preTime = new int[numPrecond];
                derived = new boolean[numPrecond];
...
//		return new GMathForm(c_dynamic);
    ...
    public void isDerp(int k) {
		derived[k] = true;
    }
    public boolean isDP(int k){
        return derived[k];
    }
